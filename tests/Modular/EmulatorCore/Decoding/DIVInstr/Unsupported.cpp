...
TEST_F(DECODER_TEST, DIV_INSTR_IMM_B8) {
  decoder.mem_controller->Load(counter, HyperCPU::Opcode::DIV);
  counter += 2;
...
TEST_F(DECODER_TEST, DIV_INSTR_IMM_B16) {
  decoder.mem_controller->Load(counter, HyperCPU::Opcode::DIV);
  counter += 2;
  decoder.mem_controller->Load(counter, EncodeTestFlags(HyperCPU::Mode::b16, HyperCPU::OperandTypes::IMM));
  ++counter;
...
TEST_F(DECODER_TEST, DIV_INSTR_IMM_B32) {
  decoder.mem_controller->Load(counter, HyperCPU::Opcode::DIV);
  counter += 2;
  decoder.mem_controller->Load(counter, EncodeTestFlags(HyperCPU::Mode::b32, HyperCPU::OperandTypes::IMM));
  ++counter;
