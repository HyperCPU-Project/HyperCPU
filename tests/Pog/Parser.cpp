...
TEST_F(TestParser,
  ...
  p.rule("strings")
      .production("strings", "string", [](Parser<Value>&, auto&& args) -> Value {
        std::get<std::vector<std::pair<std::string, std::string>>>(args[0].value).push_back(std::get<std::pair<std::string, std::string>>(args[1].value));
        return std::move(args[0].value);
      })
...
TEST_F(TestParser,
  ...
  p.set_start_symbol("S");
  p.rule("S")
      .production("func", "id", [](Parser<int>&, auto&& args) { return args[0].value + args[1].value; }, "A", [&](Parser<int>&, auto&& args) {
