...
namespace pog {
  ...
  public:
    ...
    Token(std::uint32_t index, const std::string& pattern, StatesT&& active_in_states)
        : Token(index, pattern, std::forward<StatesT>(active_in_states), nullptr) {
    }
    ...
    const re2::RE2* get_regexp() const {
      return _regexp.get();
    }
    ...
    bool has_transition_to_state() const {
      return static_cast<bool>(_enter_state);
    }
